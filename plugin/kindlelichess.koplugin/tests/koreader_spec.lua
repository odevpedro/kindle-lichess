...
describe("Kindle Lichess KOReader integration", function()
    ...
    it("constructs the complete mock session and closes it cleanly", function()
        ...
        assert.is_true(bridge:simulate_chat("Bom jogo!"))
        assert.equals(1, controller.chat_unread)
        session.chat_open = true
        controller:mark_chat_read(true)
        session:_rebuild()
        assert.is_not_nil(session.chat_actions)
        assert.matches("MockOpponent: Bom jogo!", session.chat_transcript.text, 1, true)
        session:paintTo(framebuffer, 0, 0)
    ...
    it("renders and edits the offline free board without a bridge", function()
        local Blitbuffer = require("ffi/blitbuffer")
        local Controller = require("controller")
        local Screen = require("device").screen
        local Session = require("ui/session")
        ...
        assert.equals("free_board", controller.view)
        assert.is_not_nil(session.board)
        assert.is_not_nil(session.free_actions)
        local framebuffer = Blitbuffer.new(Screen:getWidth(), Screen:getHeight())
        session:paintTo(framebuffer, 0, 0)

        assert.is_true(controller:free_select_tool("erase"))
        session.board.squares.a2:onTapSquare()
        assert.is_nil(controller.free_position:piece_at("a2"))
        assert.is_true(controller:free_select_tool("wq"))
        session.board.squares.d4:onTapSquare()
        assert.equals("q", controller.free_position:piece_at("d4").type)
        assert.is_true(controller:free_import_fen("4k3/8/8/8/8/8/8/4K3 b - - 0 1"))
        assert.equals("b", controller.free_position.turn)
